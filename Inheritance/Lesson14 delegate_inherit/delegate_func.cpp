...
public:
    void entrymarks(int a, int b) // entrymarks delegate function
    {
        setmarks(a, b);
    }
    void show()
...
int main()
{
    ...
    create public method entrymarks() to set the marks, then entrymarks() method delegates
    this particular task to setmarks() in result class, as setmarks(), being private ,can be accessed
    within the same class.and then setmarks() does the job of entrymarks, thus entrymarks() works as a
    delegate function.
